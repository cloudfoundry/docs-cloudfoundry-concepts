...
The simplest way to get started is to fetch the UAA from Git, and then build and run all of the 
components that comprise the UAA and the example programs (uaa, samples/api, and samples/app) 
...
If successful, the three apps will all be running together on a single instance of Tomcat
...
This authenticates and obtains an access token from the server using the OAuth2 implicit grant, similar to the approach intended for a standalone client like `cf`.
...
The command line example in the previous section should work against the Pivotal Web Services UAA, although token encoding is unnecessary as you will not have the client secret.
...
This authenticates and obtains an access token from the server using the OAuth2 implicit grant, which is the same grant that a client like `cf` uses.
...
Deploy as normal in Tomcat or your container of choice, or execute `./gradlew run` to run it directly from `uaa` directory in the source tree.  When running with Gradle, it listens on port 8080 and the URL is `http://localhost:8080/uaa`.
